// @ts-expect-error no @types/markdown-it-for-inline.
...
const betterLink = (
  ...
  decorate: (href: string | undefined, textContent: string) => Decoration | undefined
): typeof MarkdownIt =>
  markdown.use(iterator, 'url_new_win', 'link_open', (tokens: MarkdownItToken[], index: number) => {
    const linkCloseToken = tokens.slice(index + 1).find(({ type }) => type === 'link_close');
    const indexOfLinkCloseToken = linkCloseToken ? tokens.indexOf(linkCloseToken) : -1;
